# MANN-WHITNEY U test + boxploty (multimodální, kombinace akustika + lingvistika)
...
ACOUSTIC_COMPOSITE = os.path.join(SCRIPT_DIR, "output_domain_scores_acoustic", "composite_domain_scores.csv")
LINGUISTIC_COMPOSITE = os.path.join(SCRIPT_DIR, "output_domain_scores_linguistic", "composite_domain_scores.csv")

OUTPUT_DIR = os.path.join(SCRIPT_DIR, "output_mann_whitney_multimodal")
...
def load_data():
    ...
    if not os.path.exists(ACOUSTIC_COMPOSITE):
        raise FileNotFoundError(f"Soubor nenalezen: {ACOUSTIC_COMPOSITE}")
    
    df_acoustic = pd.read_csv(ACOUSTIC_COMPOSITE)
    
    if not os.path.exists(LINGUISTIC_COMPOSITE):
        raise FileNotFoundError(f"Soubor nenalezen: {LINGUISTIC_COMPOSITE}")
    
    df_linguistic = pd.read_csv(LINGUISTIC_COMPOSITE)
    
    score_cols_a = [c for c in df_acoustic.columns if c.endswith('_score')]
    score_cols_l = [c for c in df_linguistic.columns if c.endswith('_score')]
    
    df_acoustic_subset = df_acoustic[[SUBJECT_COL, GROUP_COL] + score_cols_a]
    df_linguistic_subset = df_linguistic[[SUBJECT_COL, GROUP_COL] + score_cols_l]
...
def plot_boxplot_with_mw(df, domain_col, domain_name, mw_results, output_dir, is_acoustic=True):
    ...
    plt.tight_layout()
    out_path = os.path.join(output_dir, f'MM_boxplot_{domain_col}.pdf')
...
def main():
    ...
    for domain_col in score_cols:
        ...
        is_acoustic = domain_col in ['Respirace_score', 'Fonace_score', 
                                      'Artikulace_score', 'Prozodie_score']
        
        mw_results = mann_whitney_tests(df_all, domain_col)
        
        all_results[domain_col] = mw_results
        
        plot_boxplot_with_mw(df_all, domain_col, domain_name, mw_results, 
                            OUTPUT_DIR, is_acoustic)
